...
func main() {
	...
	var mainUrl string = "https://cdn.jsdelivr.net/gh/fawazahmed0/currency-api@1/latest/currencies/"
	...
	if checkIfArgIsCurrency(getFirstArg()) {
		if checkIfArgIsCurrency(getSecondArg()) {
			firstArg = strings.ToLower(getFirstArg())
			secondArg = strings.ToLower(getSecondArg())
			url = mainUrl + firstArg + "/" + secondArg + ".min.json"
		...
		secondArg = strings.ToLower(getSecondArg())
		thirdArg = strings.ToLower(getThirdArg())
		url = mainUrl + secondArg + "/" + thirdArg + ".min.json"
		// get the response from the link
		response, err = http.Get(url)
		if err != nil {
			fmt.Println(err)
	...
	if checkIfArgIsCurrency(getFirstArg()) {
		...
		// get the key with the name of the currency
		err = json.NewDecoder(response.Body).Decode(&result)
		if err != nil {
			fmt.Println(err)
		}
		rate := result[secondArg]
		fmt.Println("1", getFirstArg(), "=", rate, getSecondArg())
	} else if checkIfArgIsNumber(getFirstArg()) {
		var result map[string]interface{}
		// get the key with the name of the currency
		err = json.NewDecoder(response.Body).Decode(&result)
		...
		rate, err := strconv.ParseFloat(fmt.Sprintf("%v", result[thirdArg]), 64)
		if err != nil {
			fmt.Println(err)
		}
		amount, err := strconv.ParseFloat(getFirstArg(), 64)
		if err != nil {
			fmt.Println(err)
		}
		fmt.Println(amount, getSecondArg(), "=", rate*amount, getThirdArg())
	} else {
		fmt.Println("Invalid arguments")
		os.Exit(1)
	}
